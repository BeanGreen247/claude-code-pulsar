...
class TerminalPanel {
  ...
  // Pulsar inherits a stripped PATH from the desktop session; user-local bin
  // dirs (where npm/pipx CLIs land) are often missing.  Prepend the most
  // common locations so node-pty can find 'claude' without the user having
  // to set an absolute CLI path in settings.
  _augmentedPath() {
    ...
    for (const dir of extra.reverse()) {
      if (!current.includes(dir)) current.unshift(dir)
    }
