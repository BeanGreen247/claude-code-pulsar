...
class ClaudePanel {
  ...
  _buildElement() {
    ...
    header.innerHTML = `
      <span class="claude-code-title">
        ...
        Claude Code
        <span class="cc-mode-badge" id="cc-mode-badge"></span>
      </span>
      <div class="claude-code-header-controls">
        ...
        <button class="claude-btn claude-btn-icon" id="cc-about" title="About — github.com/BeanGreen247/claude-code-pulsar"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="14" height="14" aria-hidden="true"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z"/></svg></button>
    ...
    this.stopBarBtn = document.createElement('button')
    this.stopBarBtn.className = 'cc-stop-btn'
    this.stopBarBtn.textContent = 'Stop'
    this.stopBarBtn.title = 'Stop generation'
    this.stopBarBtn.style.display = 'none'
    this.stopBarBtn.addEventListener('click', () => this.connection && this.connection.interrupt())
    sendBar.appendChild(this.stopBarBtn)
    ...
    textarea.addEventListener('keydown', (e) => {
      ...
      // ArrowUp — navigate back through prompt history
      if (e.key === 'ArrowUp' && !e.ctrlKey && !e.metaKey && !e.shiftKey) {
        ...
        if (atStart || textarea.value === '') {
          ...
          if (next < this.promptHistory.length) {
            ...
            textarea.value = this.promptHistory[next]
            textarea.style.height = 'auto'
            textarea.style.height = Math.min(textarea.scrollHeight, 200) + 'px'
      ...
      if (e.key === 'ArrowDown' && !e.ctrlKey && !e.metaKey && !e.shiftKey && this.historyIndex > -1) {
        e.preventDefault()
        const prev = this.historyIndex - 1
        this.historyIndex = prev
        const val = prev === -1 ? '' : this.promptHistory[prev]
        textarea.value = val
        textarea.style.height = 'auto'
        textarea.style.height = Math.min(textarea.scrollHeight, 200) + 'px'
  ...
  _bindConnection() {
    ...
    this.subscriptions.add(
      ...
      this.connection.onStatus((ev) => this._onStatus(ev)),
    )

    // Cache CLI availability at panel level so the badge can read it even
    // when the connection has been destroyed (e.g. between chats).
    this._cliAvailable = this.connection.cliAvailable

    // Update the header badge and, if API mode, surface the in-chat notice
    this._updateModeBadge()
    if (!this._cliAvailable) {
      this._showAPIModeNotice()
    }
  }
  ...
  _onSend() {
    ...
    if (this.promptHistory[0] !== text) this.promptHistory.unshift(text)
    this.historyIndex = -1
  ...
  _onStatus(ev) {
    // Transient status messages (e.g. "retrying in 8 s…") — keep streaming active
    this._appendSystemMessage(ev.message, 'status')
    this._showThinking()
  }
  ...
  _updateModeBadge() {
    const badge = this.element && this.element.querySelector('#cc-mode-badge')
    if (!badge) return
    // _cliAvailable is set when _bindConnection runs; undefined means not yet known
    const cli = this._cliAvailable
    if (cli === undefined) { badge.textContent = ''; return }
    badge.textContent = cli ? 'CLI' : 'API'
    badge.className   = cli ? 'cc-mode-badge cc-mode-badge--cli' : 'cc-mode-badge cc-mode-badge--api'
    badge.title       = cli
      ? 'Using local claude CLI — full file editing and shell access available'
      : 'Using Anthropic API — install the claude CLI for file editing and shell access'
  }

  _showAPIModeNotice() {
    const div = document.createElement('div')
    div.className = 'cc-api-notice'
    div.innerHTML =
      '<strong>Running in API mode</strong> — the claude CLI was not found on this machine. ' +
      'File editing and shell commands are not available. ' +
      '<a class="cc-api-notice-link" href="#">Install claude CLI</a> and log in for full ' +
      'functionality (faster responses, file edits, shell access).'
    div.querySelector('.cc-api-notice-link').addEventListener('click', (e) => {
      e.preventDefault()
  ...
  _appendUserMessage(text) {
    ...
    el.innerHTML = `
      <div class="claude-msg-header">
        ...
        <div class="claude-msg-header-actions">
          <span class="claude-msg-time">${this._fmtTime(msg.timestamp)}</span>
          <button class="cc-copy-msg-btn" title="Copy message">Copy</button>
        </div>
      </div>
      <div class="claude-msg-body">${escapeHtml(text)}</div>
    `
    el.querySelector('.cc-copy-msg-btn').addEventListener('click', () => {
    ...
    el.innerHTML = `
      <div class="claude-msg-header">
        ...
        <div class="claude-msg-header-actions">
          ...
          <button class="cc-copy-msg-btn" title="Copy response">Copy</button>
        </div>
      </div>
      <div class="claude-msg-text"></div>
    `
    el.querySelector('.cc-copy-msg-btn').addEventListener('click', () => {
      const textEl = el.querySelector('.claude-msg-text')
      if (textEl) atom.clipboard.write(textEl.innerText)
    })
    this.messagesEl.appendChild(el)
    this._scrollToBottom()
    return el
  ...
  _setStreaming(val) {
    ...
    if (this.stopBarBtn) {
      this.stopBarBtn.style.display = val ? 'block' : 'none'
      this.sendBtn.style.display = val ? 'none' : 'block'
    }
