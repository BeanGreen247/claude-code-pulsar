...
class ClaudeAPIConnection {
  ...
  setDebugMode(on)        { /* no-op in API mode — no subprocess stderr to surface */ }
  ...
  _request(delayMs = 0) {
    ...
    const fire = () => {
      ...
      const timeoutMs = this._research ? 300000 : 90000
      req.setTimeout(timeoutMs, () => {
        req.destroy()
        this.isRunning = false
        this.emitter.emit('error', { type: 'timeout', message: `Request timed out after ${timeoutMs / 1000} s` })
      })
